...
class RestAtlasClient(atlasClientConf: AtlasClientConf) extends AtlasClient {
  ...
  override protected def doCreateEntities(entities: Seq[AtlasEntity]): Unit = {
    ...
    val response = client.createEntities(entitiesWithExtInfo)
    try {
      if (response.getCreatedEntities != null) {
        logInfo(s"Entities ${response.getCreatedEntities.asScala.map(_.getAttribute("qualifiedName")).mkString(", ")} " +
          s"created")
      }

      if (response.getUpdatedEntities != null) {
        logInfo(s"Entities ${response.getUpdatedEntities.asScala.map(_.getAttribute("qualifiedName")).mkString(", ")} " +
          s"updated")
      }

      if (response.getPartialUpdatedEntities != null) {
        logInfo(s"Entities ${response.getPartialUpdatedEntities.asScala.map(_.getAttribute("qualifiedName")).mkString(", ")} " +
          s"partially updated")
      }

      if (response.getDeletedEntities != null) {
        logInfo(s"Entities ${response.getDeletedEntities.asScala.map(_.getAttribute("qualifiedName")).mkString(", ")} " +
          s"deleted")
      }

    } catch {
      case e: Throwable => throw new IllegalStateException(s"Fail to get create entities", e)
    }
  }
